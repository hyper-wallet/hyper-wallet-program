...
export const testTransferSPL = () => {
    ...
    it("Can receive SPL from another NORMAL wallet", async () => {
        ...
        assert.isAtLeast(Number(hyperWalletSPLBalance.value.amount), 1 * Math.pow(10, SPL_TOKEN_DECIMALS));
    });
    ...
    it("Can send SPL to another NORMAL wallet", async () => {
        ...
        const signature = await program.methods.transferSpl(amountToSend).accounts(({
        ...
        assert.isAtLeast(Number(normalWalletSPLBalance.value.amount), amountToSend.toNumber());
    });
    ...
    it("Can send SPL to another HYPER wallet", async () => {
        ...
        const receiverHyperWalletBalance = (await getSPLBalance(receiverHyperWalletATA.address));
        assert.isAtLeast(Number(receiverHyperWalletBalance.value.amount), amountToSend.toNumber());
    });
}
